...
echo  [8]  ^|  Firewall configuration                     [18]  ^|  WinRe (Recovery)
...
if %choice% equ "1" (
    ...
    reg add "HKEY_CLASSES_ROOT\Directory\shell\TakeOwnership" /v "Position" /t REG_SZ /d "" /f > nul 2>&1

    reg add "HKEY_CLASSES_ROOT\Directory\shell\TakeOwnership\command" /ve /t REG_SZ /d "powershell -windowstyle hidden -command Start-Process cmd.exe -ArgumentList '/c takeown /f \\\"%%1\\\" && icacls \\\"%%1\\\" /grant *S-1-3-4:F /t /c /l' -Verb runAs" /f > nul 2>&1

    reg add "HKEY_CLASSES_ROOT\Directory\shell\TakeOwnership\command" /v "IsolatedCommand" /t REG_SZ /d "powershell -windowstyle hidden -command Start-Process cmd.exe -ArgumentList '/c takeown /f \\\"%%1\\\" && icacls \\\"%%1\\\" /grant *S-1-3-4:F /t /c /l' -Verb runAs" /f > nul 2>&1
...
echo  [*INFO] Rebooting to Recovery . . .
